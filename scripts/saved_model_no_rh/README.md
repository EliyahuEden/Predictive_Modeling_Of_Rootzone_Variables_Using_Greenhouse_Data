# Rootzone pH and EC Prediction Package - NoRH

This folder contains everything needed to run the final no-RH unified 48H rootzone model.

## Folder contents

- `predict_rootzone.ipynb` - notebook used for prediction
- `master.csv` - input data file used by default
- `v8_unified_model_48h_no_rh_shared_model.joblib` - trained hybrid model
- `v8_unified_model_48h_no_rh_model_meta.json` - model metadata and feature list

Keep the notebook, CSV, model file, and metadata file together in the same folder.

## What changed in the no-RH model

The previous 48H model used `internal_rh_%` to calculate VPD, and VPD was then used in two important features.

This no-RH model removes that dependency. It replaces the old VPD calculation with a climate-demand proxy calculated from:

- `internal_air_temp_c`
- `internal_radiation`
- `ET0`

The final package also keeps short-term salt carryover context so high-EC shock intervals can be handled without using RH.

The proxy is:

```text
saturation_vapor_pressure(temp) * (0.35 + log1p(radiation_per_hour) / 8) + 0.10 * ET0_per_hour
```

The model does not require `internal_rh_%` in the prediction CSV.
...
The 48-hour pre-anchor history is needed because the model uses irrigation, fertilizer, climate, and crop history before the anchor and before the target.

For example, irrigation or fertilizer 36 hours before the anchor is used correctly if the CSV contains the full 48-hour history window.
